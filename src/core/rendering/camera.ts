...
export class Camera {
  ...
  constructor(options: CameraOptions) {
    const { fov, aspect, zNear, zFar, position, rotation, up, speed } =
  ...
  public calculateMatrix(): void {
    this._projectionMat = calculateProjectionMat(
      this._fov,
      this._aspect,
      this._zNear,
      this._zFar
  ...
  public calculateFront(): void {
    const yaw: number = toRadians(this._rotation[0]);
    const pitch: number = toRadians(this._rotation[1]);
